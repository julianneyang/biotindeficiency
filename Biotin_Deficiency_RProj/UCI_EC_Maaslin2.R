...
input_data <- read.csv("UCI/pathway-table/UCI PICRUST - uci_ec.csv", header=TRUE, row.names=1) # choose filtered non rarefied csv file
...
fit_data = Maaslin2(input_data=df_input_data, input_metadata=df_input_metadata, 
                    output = "UCI/pathway-table/EC_intestines_Maaslin2_Genotype", 
...
fit_data = Maaslin2(input_data=df_input_data, input_metadata=df_input_metadata, 
                    output = "UCI/pathway-table/ec_stool_Maaslin2_Timepoint_Genotype_1-MouseID", 
                    fixed_effects = c("Timepoint","Genotype"), 
                    random_effects=c("MouseID"),
                    min_prevalence = 0.375,
                    normalization="TSS", transform ="log",plot_heatmap = FALSE,plot_scatter = FALSE)
fit_data = Maaslin2(input_data=df_input_data, input_metadata=df_input_metadata, 
                    output = "UCI/pathway-table/ec_stool_Maaslin2_Timepoint_Genotype_TimepointANDGenotype_1-MouseID", 
...
### Evaluate results ---
# intestines ---
data<-read.table("UCI/pathway-table/EC_intestines_Maaslin2_Genotype/significant_results.tsv", header=TRUE)
...
write.csv(data, "UCI/pathway-table/intestines_significant_annotated_ec.csv")


# stool, Timepoint + Genotype ---
data<-read.table("UCI/pathway-table/ec_stool_Maaslin2_Timepoint_Genotype_1-MouseID/all_results.tsv", header=TRUE)
data <- data %>% filter(qval <0.05)

#No significant features

# stool, Timepoint*Genotype ---
data<-read.table("UCI/pathway-table/ec_stool_Maaslin2_Timepoint_Genotype_TimepointANDGenotype_1-MouseID/significant_results.tsv", header=TRUE)
data <- data %>% filter(qval <0.05)

#No significant features

### Make Volcano plot ---
data<-read.table("UCI/pathway-table/EC_intestines_Maaslin2_Genotype/all_results.tsv", header=TRUE)
annotation$feature <- gsub(":",".",annotation$feature)
data<-merge(data,annotation,by="feature")
...
uci_colon_ec <- uci_colon + labs(title = "Colon: KO vs WT",
                                   subtitle = NULL,
                                   caption = "# Significant EC: 500") + 
  theme_cowplot(16) +
...
plot_grid(ucla_cecum_ec, uci_colon_ec,ncol=2, labels=c("A","B"))
dev.new(width=15, height=10)
plot_grid(ucla_cecum_pwy, uci_colon_pwy,ncol=2, labels=c("C","D"))
